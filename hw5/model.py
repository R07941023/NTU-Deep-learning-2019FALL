...
class Fully(nn.Module):
    def __init__(self, datasize):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=datasize, out_features=1024),
...
class mini_LSTM(nn.Module):
    def __init__(self, embedding, embedding_dim, hidden_dim, num_layers, dropout=0.5, fix_emb=True):
        super(mini_LSTM, self).__init__()
        # Create embedding layer
        self.embedding = t.nn.Embedding(embedding.size(0),embedding.size(1))
        self.embedding.weight = t.nn.Parameter(embedding)
        # Fix/Train embedding
...
class bi_LSTM(nn.Module):
    def __init__(self, embedding, embedding_dim, hidden_dim, num_layers, dropout=0.5, fix_emb=False):
